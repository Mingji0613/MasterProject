...
library(ggrepel)
library(infotheo)
feat <-read.csv("C:/Users/14529/Desktop/DE_OTU.csv",header=T,row.names=1)
metaTable <-read.csv("C:/Users/14529/Desktop/DE_Y.csv",header=T,row.names=1)
...
for (f in row.names(featTable)) {
  ...
  x <- as.numeric(featTable[f, metaTable %>% 
                                filter(PDAC=="1") %>% pull(ID)])
  y <- as.numeric(featTable[f, metaTable %>% 
                                filter(PDAC=="0") %>% pull(ID)])
...
#BH½ÃÕý
#p.adj <- data.frame(apply(p.val, MARGIN=2, FUN=p.adjust, method="BH"),
#check.names = FALSE)
...
filename=p.adj1
write.csv(filename, file = '/Users/14529/Desktop/pvalue.csv')
